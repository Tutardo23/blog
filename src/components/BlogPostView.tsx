...
const myPortableTextComponents: PortableTextComponents = {
  ...
  marks: {
    ...
    link: ({ children, value }) => (
      <a
        href={value?.href}
        className="text-[#DB2777] underline decoration-wavy underline-offset-4 hover:text-[#831843]"
      >
        {children}
      </a>
    ),
  ...
  return (
    <article className="min-h-screen w-full bg-[#FFF5F7] pb-20">
      <div className="relative h-[60vh] w-full">
        ...
        <Link
          href="/"
          className="absolute top-24 left-6 md:left-12 z-20 flex items-center gap-2 rounded-full bg-white/80 backdrop-blur px-4 py-2 text-xs font-bold uppercase tracking-widest text-[#831843] hover:bg-white transition-all shadow-sm"
        >
