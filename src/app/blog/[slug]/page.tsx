...
// 1. Configuración de Estilos del Texto (La magia del Marker)
const myPortableTextComponents: PortableTextComponents = {
  block: {
    // Párrafos normales: Tipografía Serif elegante (como libro)
    normal: ({ children }) => (
  ...
  marks: {
    // 🖍️ EFECTO FELPA / MARCADOR (Usamos 'strong' / negrita para esto)
    strong: ({ children }) => (
      <span className="relative inline-block px-1 mx-1 font-bold text-[#831843] z-10">
        {/* El trazo de la felpa (atrás del texto) */}
...
// 2. Función para buscar el Post por su URL (Slug)
...
// 3. La Página del Post
export default async function BlogPost({ params }: { params: { slug: string } }) {
  const post = await getPost(params.slug);

  if (!post) return (
    <div className="min-h-screen flex items-center justify-center text-[#831843]">
  ...
  return (
    <article className="min-h-screen w-full bg-[#FFF5F7] pb-20">
      ...
      <div className="relative h-[60vh] w-full">
        ...
        {/* Gradiente para que se lea el texto encima */}
        <div className="absolute inset-0 bg-gradient-to-t from-[#FFF5F7] via-[#FFF5F7]/20 to-transparent" />
        
        {/* Botón Volver */}
        <Link href="/" className="absolute top-8 left-6 md:left-12 z-20 flex items-center gap-2 rounded-full bg-white/80 backdrop-blur px-4 py-2 text-xs font-bold uppercase tracking-widest text-[#831843] hover:bg-white transition-all">
      ...
      <div className="container mx-auto max-w-3xl px-6 -mt-32 relative z-10">
        ...
        {/* CUERPO DEL TEXTO (Con efecto felpa) */}
